...
sub main {
	...
	my $appt = $CGI->param("datetimepicker");  ## Get the appointment date and time
	my $desc = $CGI->param("description");     ### Get the description of the appointment
	...
	my $sth = $dbh->prepare(qq{INSERT INTO appointments (appointmenttime,Description) VALUES(STR_TO_DATE(?, '%Y/%m/%d %H:%i'), ?)});
...
"An error occurred during the creation of the appointment. Please check the server log file\n!";
	...
	else {  # If the insert succeeds then give a success message and siplay the form again
...
sub validate_form {
	...
	if ($appointment_value) {
		...
		my $appt_time = $dt->epoch;  # Get the appointment time since epoch (Jan 1 1970)
...
"Appointment cannot be made in the past. Please check appointment date and time <br /><br />";
		...
		$error_message .= "Appointment time cannot be empty <br /> <br />"; # Display error if the appointment time is empty
	}

    ## Display error if appointment description is empty
	if ( !$description_value ) {
		$error_message .= "Appointment description cannot be empty <br /> <br />";
	}
