...
qq{SELECT appointmenttime,Description FROM appointments WHERE Description LIKE ?}
		...
		if (!$result1) {
			die "An error occurred during the search. Please see the server log file";
		}
		...
		$sth1 =
		  $dbh->prepare(
			qq{SELECT appointmenttime,Description FROM appointments});  ## if the search string is empty then get all the database records
